...
$reportId = $_GET['id'];
//========================================== SSLR Report Export ================================================//
$objPHPExcel->getActiveSheet()->getCell('B2')->setValue('Stop the Job Meeting Review');    
$objPHPExcel->getActiveSheet()->getStyle('A6:Q6')->applyFromArray($styleArray);
...
$teamNameQuery = "SELECT team_name from TEAM  where team_id IN (Select team_id from STJ where stj_id = '{$reportId}')";
...
$companyNameQuery = "SELECT company_name from COMPANY  where company_id IN (Select company_id from STJ where stj_id = '{$reportId}')";
...
$query = "Select * from STJ where stj_id = '{$reportId}';";
...
while($row=pg_fetch_row($export))
{
	...
	$objPHPExcel->getActiveSheet()->SetCellValue('E'.$rowcount1,$row[18]); // created by
	$objPHPExcel->getActiveSheet()->SetCellValue('F'.$rowcount1,$row[5]); // created on
	$objPHPExcel->getActiveSheet()->SetCellValue('G'.$rowcount1,$row[19]); // updated by
	$objPHPExcel->getActiveSheet()->SetCellValue('H'.$rowcount1,$row[4]); // updated on
	$objPHPExcel->getActiveSheet()->SetCellValue('I'.$rowcount1,$row[13]); // comments
	$objPHPExcel->getActiveSheet()->SetCellValue('J'.$rowcount1,$row[6]); // reason
	$objPHPExcel->getActiveSheet()->SetCellValue('K'.$rowcount1,$row[15]); // text note
	$objPHPExcel->getActiveSheet()->SetCellValue('L'.$rowcount1,$row[7]); // Question 1
	$objPHPExcel->getActiveSheet()->SetCellValue('M'.$rowcount1,$row[9]); // Question 2
...
$objPHPExcel->getActiveSheet()->getStyle('A6:Q6')->applyFromArray($BStyle);
$objPHPExcel->getActiveSheet()->getStyle('A7:Q7')->applyFromArray($BStyle);
$objPHPExcel->getActiveSheet()->getStyle('A6:Q6')->applyFromArray($BiStyle);
$objPHPExcel->getActiveSheet()->getStyle('A7:Q7')->applyFromArray($BiStyle);
//===============================================================================================================//

ob_end_clean();
ob_start();
header('Content-Type: application/octet-stream');
header("Content-Disposition: attachment; filename=Stop the Job Meeting Review.xls");
