...
function assignAssetToUser(value){
    var checkBoxId = "checkbox"+value;
    var checkBox = document.getElementById(checkBoxId);
	if(checkBox.checked){ // if user seleced for asset assignment
		$.post("adminFiles/assetAssignmentScript.php",{userIdForAssetAssignment:value}, function(x){
		...
		$.post("adminFiles/assetAssignmentScript.php",{userIdForAssetRemoval:value}, function(x){
			if(x=="success"){     // x contains response of the function
			    alert("Asset removed!")
				$("#pageContent").load("adminFiles/assignAssetUsers.php"); 
			}
...
<ul class="breadcrumb" align="right"; style="background:rgba(255, 255, 255, .0);!important ">
	...
	<li>
	  <a onClick="" style="color:grey;">
	    <i class="fa fa-check" aria-hidden="true"></i> Assign user
	  </a>
	</li>
</ul>
	  ...
	  <div class="table-responsive">
		<table id="example1"  class="table table-bordered table-striped">
		  ...
		  <tbody style="margin-bottom:10px; !important">
			<?php 
			$adminActionManager-> getUsersForAssetAssignment();
			?>
		  </tbody>
		</table>
	  </div>
